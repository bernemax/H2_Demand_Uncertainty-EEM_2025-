...
*Cap_H2_pipe_PCI(p_h2) + Pipe_Cap_up_to_Max_ENTSOG(p_H2) 
;
H2_demand(t,n)  =Scenario_H2_demand(n,s) /  8760
;
*Cap_import_terminal_I.up(n) = Max_Cap_H2_Terminal(n)
*;
*Cap_res.up(res) = 0.400
*;
*Cap_line_EL.up(l) = Ex_line_cap(l) 
*;
Option LP = Gurobi
;
*$ontext
...
$ontext
Report_costs(*,*,*)
Report_Transport_losses(s,p_h2,*)
Report_pipeline_expansion(p_h2,*)
Report_Import_terminal_expansion(n,*)
Report_electrolyzer_expansion(h,*)
Report_OCGT_expansion(h,*)
Report_H2_Storage_expansion(h,*)
Report_H2_total_flow(s,p_h2,*)
Report_H2_total_import(s,n,*)
Report_H2_total_production(s,h,*)
Report_Cap_Res(res,*)
Report_import_share(s,n)
Report_Ex_Im_detail(s,nn,n,r,c,d)
Report_total_Ex_IM(s,nn,n)
Report_total_Gen(*,*)
Saldo_H2_Stor(*,*)
Gen_H2_Stor(*,*)
...
;
$ontext
Report_scenarios(s,p_h2,'Expansion in GWh')= round(Cap_pipe_H2.l(p_h2) * 1000  + EPS ,3)
;
Report_scenarios(s,n,'Long Terminal in GWh')$Import_countries(n) = round(Cap_import_terminal_I.l(n)  * 1000  + EPS ,3)
;
Report_scenarios(s,b,'Battery Storage Cap in GWh') = round(Cap_battery_M.l(b) * 6 * 1000 + EPS ,3)
;
Report_scenarios(s,b,'Battery Inverter Cap in GW') = round(Cap_battery_M.l(b) * 1000 + EPS ,3)
;
Report_scenarios(s,h,'Electrolyzer Cap in GW') = round(Cap_ELEC_H2.l(h) * 1000 + EPS ,3)
;
Report_scenarios(s,h,'OCGT Cap in GW') = round(CAP_OCGT_EL.l(h) * 1000 + EPS ,3)
;
Report_scenarios(s,h,'Hydrogen Storage Cap in GWh') = round(Cap_Stor_h2.l(h) * 168  * 1000 + EPS ,3) * scale
;
Report_scenarios(s,solar_pv,'solar PV in GW') = round(Cap_res.l(solar_PV) * 1000 + EPS ,3)
;
Report_scenarios(s,wind,'wind onshore in GW') = round(Cap_res.l(wind)$Wind_on(wind) * 1000 + EPS ,3)
;
Report_scenarios(s,wind,'wind offshore in GW') = round(Cap_res.l(wind)$Wind_off(wind) * 1000 + EPS ,3)
;
Report_scenarios(s,l,'New Line Capacity in GW') = round(Cap_line_EL.l(l) * 1000 + EPS ,3)
;
Report_scenarios(s,n,'Total Terminal in GWh') = round((Cap_import_terminal_II.l(n) + Cap_import_terminal_I.l(n))  * 1000  + EPS ,3)
;
Report_scenarios(s,p_h2,'H2_total_flow TWh') = round(sum(t, abs(Flow_H2.l(t,p_h2))) ,3) * scale
;
Report_scenarios(s,n,' total import in TWh') = round(sum(t, import_H2_I.l(t,n) +  import_H2_II.l(t,n))  ,3) * scale
;
Report_scenarios(s,h,'Total H2 Generation in TWh') = round(sum(t,Gen_Elec_h2.l(t,h) ) ,3) * scale
;
Report_scenarios(s,solar_pv,'Total SOlar PV Generation in TWh') = round(sum(t,Gen_Res.l(t,solar_pv) ) ,2) * scale
;
Report_scenarios(s,wind,'Total Wind Generation ONshore in TWh') = round(sum(t,Gen_Res.l(wind)$Wind_on(wind) ) ,2) * scale
;
Report_scenarios(s,wind,'Total Wind Generation Offshore in TWh') = round(sum(t,Gen_Res.l(t,wind)$Wind_off(wind) ) ,2) * scale
;
Report_scenarios(s,n,'Total EL shed in TWh') = round(sum(t,Shed_EL.l(t,n) ) ,2) * scale
;
Report_scenarios(s,p_h2,'Transport_losses in GWh') = round(sum(t, abs(Flow_H2.l(t,p_h2)) * 0.05 * H2_pipe_lengh(p_H2)) *1000   ,3) * scale
;
Report_pipeline_expansion(p_h2,'GWh/d') = round(Cap_pipe_H2.l(p_h2) * 1000 * (24/scale)  ,3)
;
Report_pipeline_expansion(p_h2,'PCI in GWh/d') = round(Cap_H2_pipe_PCI(p_h2) * 1000 * (24/scale)   ,3)
;
Report_Import_terminal_expansion(n,'GWh/d')$Import_countries(n) = round(Cap_import_terminal_I.l(n)  * 1000 * (24/scale)  ,3)
;
Report_Import_terminal_expansion(n,'PCI GWh/d')$Import_countries(n) = round(Cap_H2_Terminal_PCI(n)  * 1000 * (24/scale)  ,3)
;
Report_H2_total_import(s,n,'TWh') = round(sum(t, import_H2_I.l(s,t,n) +  import_H2_II.l(s,t,n))  ,3) * scale
;
Report_electrolyzer_expansion(h,'Cap in GW') = round(Cap_ELEC_H2.l(h) * 1000 ,3)
;
Report_electrolyzer_expansion(h,'PCI Cap in GW') = round(Cap_ELectrolyzer_PCI(h) * 1000 ,3)
;
Report_OCGT_expansion(h,'Cap in GW') =   round(CAP_OCGT_EL.l(h) * 1000 ,3)
;
Report_H2_Storage_expansion(h,'H2 Stor Cap in GWh') = round(Cap_Stor_h2.l(h) * 168 * 1000 ,3)
;
Report_H2_Storage_expansion(h,'PCI H2 Stor Cap in GWh ') = round(Cap_H2_Storage_PCI(h) * 1000 ,3)
;
Report_H2_total_flow(s,p_h2,'TWh') = round(sum(t,abs(Flow_H2.l(s,t,p_h2)))  ,3) * scale
;
Report_H2_total_production(s,h,'TWh') = round(sum(t, Gen_Elec_h2.l(s,t,h) )  ,3) * scale
;
Report_Cap_Res(solar_pv,'solar PV in GW') = round(Cap_res.l(solar_PV) * 1000 ,3)
;
Report_Cap_Res(wind,'wind onshore in GW') = round(Cap_res.l(wind)$Wind_on(wind) * 1000 ,3)
;
Report_Cap_Res(wind,'wind offshore in GW') = round(Cap_res.l(wind)$Wind_off(wind) * 1000 ,3)
;
...
;
$ontext
Report_Ex_Im_detail(s,nn,n,r,c,d) = round(sum(t, export_I.l(s,t,nn,n,r,c,d)$Map_Ex_Im(nn,n) + export_II.l(s,t,nn,n,r,c,d)$Map_Ex_Im(nn,n) ),3) * scale
;
...
;
execute_unload "Results_Loop_Det_EEM_final.gdx";
$stop
